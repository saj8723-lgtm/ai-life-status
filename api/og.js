...
const ST=[{n:'💪',k:'hp',c:'#e84393'},{n:'🧠',k:'int',c:'#0984e3'},{n:'💰',k:'pay',c:'#f9a825'},{n:'😎',k:'solo',c:'#7c5ce7'},{n:'🔥',k:'mot',c:'#fd79a8'},{n:'🍀',k:'luk',c:'#00b894'}];
...
function genOG(name){
  ...
  const wi=Math.floor(r()*WKS.length);   // wk
  r();                                    // qt skip
...
export default async function handler(req){
  try{
    ...
    const axes=ST.map((_,i)=>{
      const angle=(i*60-90)*Math.PI/180;
      return h('line',{x1:String(cx),y1:String(cy),x2:String((cx+R*Math.cos(angle)).toFixed(1)),y2:String((cy+R*Math.sin(angle)).toFixed(1)),stroke:'rgba(124,92,231,0.2)',strokeWidth:'1'});
    });
    // グリッド
    ...
    const imgRes=new ImageResponse(
      h('div',{style:{
        ...
        // 装飾円
        h('div',{style:{position:'absolute',top:'-70px',right:'-70px',width:'280px',height:'280px',
          borderRadius:'50%',background:'rgba(124,92,231,0.07)',display:'flex'}}),
        h('div',{style:{position:'absolute',bottom:'-50px',left:'-50px',width:'220px',height:'220px',
          borderRadius:'50%',background:'rgba(255,140,66,0.07)',display:'flex'}}),

        // メインコンテンツ（2カラム）
        h('div',{style:{display:'flex',flex:1,padding:'32px 40px 28px',gap:'20px',marginTop:'7px'}},

          // ===== 左カラム =====
          h('div',{style:{display:'flex',flexDirection:'column',flex:1,justifyContent:'center'}},
            // ギルドラベル
            h('div',{style:{fontSize:'15px',color:'#8a7aaa',letterSpacing:'0.12em',marginBottom:'10px',display:'flex'}},
              '✦ 冒険者ギルド登録結果 ✦'
            ),
            // 名前
            h('div',{style:{
              fontSize:name.length>8?'54px':name.length>5?'66px':'76px',
              fontWeight:700,color:'#2a2040',
              letterSpacing:'0.03em',lineHeight:1.1,marginBottom:'8px',display:'flex'
            }},name),
            // ジョブ
            h('div',{style:{fontSize:'30px',fontWeight:700,color:jc,
              letterSpacing:'0.05em',marginBottom:'16px',display:'flex'}},
              '【'+d.job+'】'
            ),
            // レベル・称号バッジ
            h('div',{style:{display:'flex',gap:'10px',alignItems:'center',marginBottom:'20px'}},
              h('div',{style:{
                ...
                borderRadius:'10px',padding:'6px 18px',fontSize:'24px',fontWeight:700,color:'#7c5ce7',display:'flex'
              }},'Lv.'+d.lv),
              h('div',{style:{
                background:'rgba(124,92,231,0.06)',border:'1.5px solid rgba(124,92,231,0.2)',
                borderRadius:'10px',padding:'6px 18px',fontSize:'20px',fontWeight:700,color:'#7c5ce7',display:'flex'
              }},d.title+'級冒険者')
            ),
            // 必殺技
            h('div',{style:{
              background:'rgba(253,121,168,0.08)',border:'1.5px solid rgba(253,121,168,0.22)',
              ...
              h('div',{style:{fontSize:'22px',fontWeight:700,color:'#fd79a8',display:'flex'}},'「'+d.move+'」')
            ),
            // 弱点
            h('div',{style:{
              background:'rgba(42,32,64,0.04)',border:'1.5px solid rgba(42,32,64,0.1)',
              borderRadius:'10px',padding:'8px 16px',display:'flex',flexDirection:'column'
            }},
              h('div',{style:{fontSize:'13px',color:'#8a7aaa',letterSpacing:'0.08em',marginBottom:'3px',display:'flex'}},'🛡 弱点'),
              h('div',{style:{fontSize:'16px',color:'#5a4a7a',display:'flex'}},wkShort)
            )
          ),
        ...
        h('div',{style:{
          ...
          fontSize:'16px',color:'#9a8aba',letterSpacing:'0.04em',display:'flex'
    ...
    if(buf.byteLength===0)return new Response('render failed: empty buffer',{status:500,headers:{'Content-Type':'text/plain'}});
